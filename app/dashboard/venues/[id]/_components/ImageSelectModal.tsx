...
export function ImageSelectModal({
  ...
  const loadImages = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/s3/images?prefix=venues/");
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {/* Images Grid */}
        <div>
          <h3 className="text-sm font-medium mb-3">Select from Existing Images</h3>
          {loading ? (
            ...
            <div className="grid grid-cols-3 gap-3 max-h-96 overflow-y-auto">
              {images.map((image) => (
                <div
                  ...
                  className={`relative aspect-square cursor-pointer rounded-md border-2 overflow-hidden transition-all ${selectedImage === image.url
